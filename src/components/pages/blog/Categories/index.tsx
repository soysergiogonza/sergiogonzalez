...
const renderIcon = (icon: any) => {
  ...
  switch (icon.type) {
    ...
    case "external":
      return (
        <img src={icon.external.url} alt='' className={styles.categoryIcon} />
      );
    default:
      return null;
  }
...
export const Categories = () => {
  ...
  const handleCategoryClick = (
    ...
    if (!details.open && articles.length > 0) {
      const targetArticle =
        articles.find((article) => article.url === selectedArticle) ||
        articles[0];
  ...
  if (isLoading)
    return <div className={clsx(styles.loading)}>Cargando categorías...</div>;
          ...
          >
            ...
            >
              <summary className={styles.category}>
                ...
                <span>{category}</span>
                {articles.length > 0 && (
                  <span className={styles.articleCount}>{articles.length}</span>
                )}
              </summary>
